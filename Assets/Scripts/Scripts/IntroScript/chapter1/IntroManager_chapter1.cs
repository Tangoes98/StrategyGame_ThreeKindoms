...
public class IntroScripts
{
    public List<IntroScript> introScripts;
}
...
public class IntroManager_chapter1 : MonoBehaviour
{
    //用于检测鼠标是否位于按钮所在区域
    ...
    public void ParseData()
    {
        ...
        dialogues = JsonMgr.Instance.LoadData<IntroScripts>("IntroScript/introScript1");
        //将数据集合按照ID号分别放入
    ...
    private void PressMouse()
    {
        if (!hitButton && !MenuPanelIsOpen)
        {
            ...
            {
                SceneMgr.GetInstance().LoadSceneAsyn("Story1", () =>
                {
