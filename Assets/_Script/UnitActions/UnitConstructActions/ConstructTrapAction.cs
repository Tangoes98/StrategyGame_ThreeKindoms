...
public class ConstructTrapAction : UnitBaseConstructAction
{
    [Header("Action_Trap")]
    [SerializeField] Transform _TrapConstructionPrefab;
    ...
    void BuildTrap(Vector3 worldPosition)
    {
        Transform roadTransform = Instantiate(_TrapConstructionPrefab, worldPosition, Quaternion.identity);
        Construction trap = roadTransform.GetComponent<Construction>();

        GridPosition gridPosition = LevelGrid.Instance.GetGridPosition(worldPosition);
        LevelGrid.Instance.AddConstructionToGrdObject(gridPosition, trap);
    }
